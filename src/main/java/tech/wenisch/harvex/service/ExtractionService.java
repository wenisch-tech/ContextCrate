...
public class ExtractionService {
  ...
  public void extract(PipelinePayload payload) {
    ...
    List<ExtractionRule> enabled = rules.findByEnabledTrueOrderByNameAsc();
  ...
  public ExtractionRule createRule(
      String name, PipelineTypes.ExtractionType type, String pattern, boolean enabled) {
  ...
  public ExtractionRule updateRule(
      UUID id, String name, PipelineTypes.ExtractionType type, String pattern, boolean enabled) {
    ExtractionRule rule = rules.findById(id).orElseThrow();
  ...
  public Page<ExtractionResult> search(
    ...
    return results.search(runId, documentId, chunkId, ruleId, blankToNull(value), pageable);
  }

  @Transactional
  public void rebuildRun(UUID runId) {
    results.deleteByRunId(runId);
    for (NormalizedDocument document : documents.findByRunId(runId)) publish(document, true);
  }

  @Transactional
  public void rebuildDocument(UUID documentId) {
    NormalizedDocument document = documents.findById(documentId).orElseThrow();
    results.deleteByDocumentId(documentId);
    publish(document, true);
  }

  public List<ExtractionRule> rules() {
    return rules.findAllByOrderByCreatedAtDesc();
  }
  ...
  public void publish(NormalizedDocument document, boolean rebuild) {
    ...
    queue.publish(
        PipelineMessage.create(
            WorkStage.EXTRACT,
            JobService.payload(document.getRunId(), document.getId()),
            document.getRunId(),
            key,
            30));
  }
