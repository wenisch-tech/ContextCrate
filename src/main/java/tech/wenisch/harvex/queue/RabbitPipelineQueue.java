...
import tech.wenisch.harvex.domain.PipelineTypes.WorkStatus;
import tech.wenisch.harvex.domain.PipelineWorkItem;
import tech.wenisch.harvex.repository.PipelineWorkItemRepository;
...
public class RabbitPipelineQueue implements PipelineQueue {
  ...
  public RabbitPipelineQueue(
      RabbitTemplate rabbit, PipelineWorkItemRepository repository, HarvexProperties properties) {
    this.rabbit = rabbit;
  ...
  public Optional<PipelineMessage> claim(WorkStage stage) {
    rabbit.receiveAndConvert("harvex." + key(stage), 1);
    var rows =
