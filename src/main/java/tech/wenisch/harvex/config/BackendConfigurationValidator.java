...
public class BackendConfigurationValidator implements ApplicationRunner {
  ...
  public BackendConfigurationValidator(HarvexProperties properties) {
  ...
  public void run(ApplicationArguments args) {
    ...
    if (properties.queue().backend().equals("local") && !properties.role().equals("all")) {
      throw new IllegalStateException(
          "harvex.queue.backend=local requires harvex.role=all; use RabbitMQ for independent worker"
              + " processes");
    }
    if (properties.database().backend().equals("h2") && !properties.role().equals("all")) {
      throw new IllegalStateException(
          "File-backed H2 requires a single process with harvex.role=all");
    }
  }
