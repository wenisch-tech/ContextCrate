...
final class RerankingSearchSupport {
  ...
  static List<SearchIndex.SearchHit> apply(SearchIndex.SearchRequest request,List<SearchIndex.SearchHit> hits,RerankingProvider provider){
    ...
    try {List<Float> scores=provider.rerank(request.query(),hits.stream().map(h->h.retrievalContent()==null?h.snippet():h.retrievalContent()).toList());if(scores.size()!=hits.size())throw new IllegalStateException("Reranker returned an unexpected score count");List<SearchIndex.SearchHit> scored=new ArrayList<>();for(int i=0;i<hits.size();i++){var h=hits.get(i);float score=scores.get(i);scored.add(new SearchIndex.SearchHit(h.id(),h.documentId(),h.runId(),h.kind(),h.title(),h.sourceUri(),h.chunkOrdinal(),h.snippet(),score,h.lexicalScore(),h.semanticScore(),score,h.content(),h.retrievalContent()));}return scored.stream().sorted(Comparator.comparing(SearchIndex.SearchHit::score).reversed().thenComparing(h->h.id().toString())).limit(request.limit()).toList();}catch(Exception e){log.warn("Reranking failed for crate {}; using retrieval order: {}",request.crateId(),e.getMessage());return hits.stream().limit(request.limit()).toList();}
  }
}
