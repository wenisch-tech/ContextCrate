...
public class ExtractionApiController {
  ...
  public ExtractionApiController(ExtractionService service, CrateAccessService access) {
    this.service = service;
    this.access = access;
  }

  @GetMapping("/extraction-rules")
  public List<RuleResponse> rules(@PathVariable UUID crateId) {
    access.require(crateId, CrateMember.Role.VIEWER);
    return service.rules(crateId).stream().map(this::rule).toList();
  }

  @PostMapping("/extraction-rules")
  @ResponseStatus(HttpStatus.CREATED)
  public RuleResponse createRule(@PathVariable UUID crateId, @RequestBody RuleRequest request) {
    access.requireMutable(crateId, CrateMember.Role.EDITOR);
    return rule(
        service.createRule(crateId, request.name(), request.type(), request.pattern(), request.enabled()));
  }

  @PutMapping("/extraction-rules/{id}")
  public RuleResponse updateRule(@PathVariable UUID crateId, @PathVariable UUID id, @RequestBody RuleRequest request) {
    access.requireMutable(crateId, CrateMember.Role.EDITOR);
    return rule(
        service.updateRule(crateId,id, request.name(), request.type(), request.pattern(), request.enabled()));
  }

  @DeleteMapping("/extraction-rules/{id}")
  @ResponseStatus(HttpStatus.NO_CONTENT)
  public void deleteRule(@PathVariable UUID crateId, @PathVariable UUID id) {
    access.requireMutable(crateId, CrateMember.Role.EDITOR);
    service.deleteRule(crateId,id);
  }

  @PostMapping("/extraction-rules/test")
  public List<MatchResponse> test(@PathVariable UUID crateId, @RequestBody TestRuleRequest request) {
    access.requireMutable(crateId, CrateMember.Role.EDITOR);
  ...
  public Page<ResultResponse> ruleResults(
    ...
    access.require(crateId, CrateMember.Role.VIEWER);
    return service.search(crateId, runId, documentId, chunkId, ruleId, value, pageable).map(this::result);
  }
  ...
  public void rebuildRun(@PathVariable UUID crateId, @PathVariable UUID runId) {
    access.requireMutable(crateId, CrateMember.Role.EDITOR);
    service.rebuildRun(crateId,runId);
  }

  @PostMapping("/documents/{documentId}/extractions/rebuild")
  @ResponseStatus(HttpStatus.ACCEPTED)
  public void rebuildDocument(@PathVariable UUID crateId, @PathVariable UUID documentId) {
    access.requireMutable(crateId, CrateMember.Role.EDITOR);
    service.rebuildDocument(crateId,documentId);
  }
