...
public class DocumentParser {
  ...
  public void parse(PipelinePayload payload) throws Exception {
    ...
    if (payload.crateId() != null
        && (!payload.crateId().equals(fetch.getCrateId()) || !payload.crateId().equals(run.getCrateId())))
    ...
    if (text.length() < 100
      ...
      queue.publish(
          PipelineMessage.create(
              ...
              JobService.payload(run.getCrateId(), run.getId(), frontierId),
              run.getId(),
              run.getCrateId() + ":browser:" + frontierId,
              80));
      return;
    }
    ...
    var documentValue =
        new NormalizedDocument(
                ...
                Hashing.sha256(text),
                metadata);
    documentValue.assignCrate(run.getCrateId());
    var document = documents.save(documentValue);
    ...
    queue.publish(
        PipelineMessage.create(
            run.getCrateId(),
            WorkStage.INDEX,
            JobService.payload(run.getCrateId(), run.getId(), document.getId()),
            run.getId(),
            run.getCrateId() + ":index:" + document.getId(),
            20));
  }
  ...
  private void discover(
    ...
    for (Element link : page.select("a[href]")) {
      ...
      try {
        var entry = new FrontierEntry(UUID.randomUUID(), run.getId(), raw, canonical, depth + 1);
        entry.assignCrate(run.getCrateId());
        ...
        queue.publish(
            PipelineMessage.create(
                run.getCrateId(),
                WorkStage.FETCH,
                JobService.payload(run.getCrateId(), run.getId(), entry.getId()),
                run.getId(),
                run.getCrateId() + ":fetch:" + entry.getId(),
                10));
  ...
  private static List<DocumentChunk> chunk(
    ...
    while (start < text.length()) {
      ...
      if (!value.isBlank()) {
        ...
        chunk.assignCrate(doc.getCrateId());
        result.add(chunk);
      }
