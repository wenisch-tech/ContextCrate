...
ContextCrate calls `POST {base-url}/embeddings` using the OpenAI request/response format. This works with OpenAI and compatible self-hosted servers. The API key is sent as a Bearer credential and must be injected from a secret store, never job JSON or version control. The text being embedded is sent to the endpoint, so use TLS and choose an endpoint consistent with your data-boundary policy. `CONTEXTCRATE_EMBEDDINGS_OPENAI_COMPATIBLE_HEADERS` accepts newline-separated additional headers for compatible gateways.
