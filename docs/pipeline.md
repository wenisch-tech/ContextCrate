...
Search is query-side retrieval over the same Lucene/OpenSearch records created by indexing. The first retrieval mode uses backend BM25 scoring across title, heading, URL text, and normalized text fields, with optional run and kind filters. Search does not create new crawl or extraction work; it reads the latest committed index.
